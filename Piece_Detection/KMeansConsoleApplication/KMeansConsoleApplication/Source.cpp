...
void ExecutePCA(const int rectEdge, const Mat& srcConverterd, Mat& imageAfterPca)
{
	...
	for (int i = 0; i < projection_result.rows; ++i)
	{
		imageAfterPca.at<Vec3f>(i / imageAfterPca.cols, i % imageAfterPca.cols).val[0] = projection_result.at<float>(i, 0);
		imageAfterPca.at<Vec3f>(i / imageAfterPca.cols, i % imageAfterPca.cols).val[1] = projection_result.at<float>(i, 1);
		imageAfterPca.at<Vec3f>(i / imageAfterPca.cols, i % imageAfterPca.cols).val[2] = projection_result.at<float>(i, 2);
	}
...
void GenerateNeightMatrix(Mat& src1)
{
	...
	Mat imageAfterPca = Mat::zeros(src1.rows, src1.cols, CV_32FC3);
