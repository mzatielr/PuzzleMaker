...
void blackenOrigImage(Mat& blackenClustered)
{
	...
	for (int i = 0; i<blackenClustered.cols*blackenClustered.rows; i++)
	{
		a = i / blackenClustered.cols;
		b = i % blackenClustered.cols;
	
		if (blackenClustered.at<float>(Point(a,b )) == static_cast<float>(neutralColor))// access violation thrown at this line, why ?
		{
			origBlacken.at<Vec3f>(a,b).val[0] = 0;
			//cout << "aaaaaaaaaaaaaaaaaaaaaaaaaaa\n";
			origBlacken.at<Vec3f>(a,b).val[1] = 0;
			origBlacken.at<Vec3f>(a,b).val[2] = 0;
		}
	...
	/*
		for ( i = 0; i < blackenClustered.rows; ++i)
		{
			for ( j = 0; j < blackenClustered.cols; ++j)
			{
				cout << j << "\\" << blackenClustered.cols << " \n";
			//	cout << j << "\\" << origBlacken.cols << " \n";


				if (blackenClustered.at<float>(Point(i, j)) == static_cast<float>( 0) )// access violation thrown at this line, why ?
				{
					origBlacken.at<Vec3f>(i, j).val[0] = 0;
					//cout << "aaaaaaaaaaaaaaaaaaaaaaaaaaa\n";
					origBlacken.at<Vec3f>(i, j).val[1] = 0;
					origBlacken.at<Vec3f>(i, j).val[2] = 0;
				}
			}
