...
int backColor = -1; // if the backColor =0 it means that the backGround is black
...
imlist color_to_bw(imlist color, int threshold){
    ...
    for(imlist::iterator i = color.begin(); i != color.end(); i++){
        ...
        cv::cvtColor(*i, bw, CV_BGR2GRAY);
			...
			cv::namedWindow("thresholding", cv::WINDOW_NORMAL);// Create a window for display.
			...
			if (a == false)
			{
			cv::(bw, bw2, 20, 255, cv::THRESH_BINARY);

			cv::imshow("bw1", bw2);
			cv::resizeWindow("bw1", 1000, 1000);
			cv::waitKethresholdy(0);
		...
		*/
			std::cout << "using thresh: " << threshold;
			if (backColor == -1)
			{
				std::cout << "\nEnter back ground color (0 is black): \n";
				std::cin >> backColor;
			}
			if (backColor == 0)
			{
			...
			{
				cv::threshold(bw, bw, threshold, 255, cv::THRESH_BINARY_INV);
