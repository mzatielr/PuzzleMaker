...
std::vector<piece> puzzle::extract_pieces(std::string path, bool needs_filter){
	...
	cv::namedWindow("original", cv::WINDOW_NORMAL);// Create a window for display.
	cv::namedWindow("median blur", cv::WINDOW_NORMAL);// Create a window for display.
...
//    cv::imwrite("/tmp/final/thresh.png", bw[0]);
    ...
    for(int i = 0; i<color_images.size(); i++){
		...
		for (int j = 0; j < contours.size(); j++){
			...
			if (a == false){
			cv::namedWindow("contours", cv::WINDOW_NORMAL);// Create a window for display.
...
//			std::this_thread::sleep_for(std::chrono::milliseconds(19999));
			if (a == false){
				...
				cv::namedWindow("mini_color", cv::WINDOW_NORMAL);// Create a window for display.
